import PacketParser
from gpsweb.models import Unit, LocationLog, Alert, AlertLog
...
def writeLocationLog(imei , data):
    ...
    locationLog.timestamp = datetime.now()
...
def checkAlerts(locationLog):
    ...
    for alert in alerts:
        if checkForTriggers(locationLog, alert):
            alertLog = AlertLog()
            alertLog.locationLog = locationLog
            alertLog.alert = alert
            alertLog.notification_sent = sendAlert(locationLog, alert)
            alertLog.save()
...
def checkForTriggers(locationLog, alert):
    ...
    if alert.type == Alert.SPEED_ALERT:
        if locationLog.speed >= alert.max_speed:
            alert_exist = True
            
    elif alert.type == Alert.GEOFENCE_ALERT:
        if PacketParser.is_in_area(locationLog, alert):
            alert_exist = True
            
    elif alert.type == Alert.SCHEDULE_ALERT:
...
def getAlertFormat(alert, recipientType):
    if alert.type == Alert.SPEED_ALERT:
        if recipientType == "email" : 
            ...
            return ""
    if alert.type == Alert.GEOFENCE_ALERT:
        if recipientType == "email" : 
            return ""
        elif recipientType == "sms" :
            return ""
    if alert.type == Alert.SCHEDULE_ALERT:
        if recipientType == "email" : 
            return ""
        elif recipientType == "sms" :
            return ""
...
def sendMail(locationLog, alert, recipient):
    ...
    mail_args = {"nickname":recipient.nickname, 
                     ...
                     "location":locationLog.lat + "%20" + locationLog.long,
...
def sendAlert(locationLog, alert):
    ...
    if alert.state + timedelta(minutes = alert.cutoff) > locationLog.timestamp :
        return False
    for recipient in alert.recipient:
        if recipient.email:
